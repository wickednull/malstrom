#!/usr/bin/env bash
# ============================================================================
# DarkSec MALSTROM — Ubuntu installer
#
# Tuned installer for Ubuntu 26.04 LTS "Resolute Raccoon" (current LTS) and
# newer.
#
# Two of MALSTROM's post-exploitation tools are packaged by Kali but NOT by
# Ubuntu (universe has neither netexec nor responder), so this installer pulls
# them from upstream and drops them on PATH where the daemon's health checks
# look for them:
...
#
#   sudo ./install-ubuntu.sh                normal install
#   sudo ./install-ubuntu.sh --start --enable-boot
#   sudo ./install-ubuntu.sh --skip-tools   don't fetch netexec/responder
#   sudo ./install-ubuntu.sh --no-deps      don't touch apt at all
...
TARGET_ID="ubuntu"
DISTRO_NAME="Ubuntu"
VALIDATED_FROM='Ubuntu 26.04 LTS "Resolute Raccoon"'
MIN_MAJOR=22
...
# --- netexec + responder (absent from the Ubuntu archive) -------------------
ensure_build_tools() {
    want=""
